...
var nav;
var imageWidths = [2436,2424,1388,800,2600,968,968,2400,2400];
var imageHeights = [2160,2160,2160,1220,2600,1864,1864,1536,1536];
   		...
   		if (Math.abs(spring.getVelocity()) < 0.05 && window.devicePixelRatio < 1.1)
			...
			$("#slides li").each(function(i, val) {
				...
				if (slideProgress > 0) { // Only bother if the slide is visible
					...
					// Filter the progress through an ease out curve and use that to control transition
					var easeOutSlideProgress = (slideProgress < 1) ? slideProgress*(2-slideProgress) : 1;
					var scale = transitionForProgressInRange(easeOutSlideProgress,0.5,1.0);
					var opacity = (transitionForProgressInRange(easeOutSlideProgress,0.5,1.0))**10;
					var blur = (1-transitionForProgressInRange(easeOutSlideProgress,0.5,1.0))*10;
					...
					val.style['filter'] = 'blur(' + blur + 'rem)';
